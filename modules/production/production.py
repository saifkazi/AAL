...
class Production(ShipmentAssignMixin, Workflow, ModelSQL, ModelView):
    ...
    inward_ref = fields.Many2One('stock.shipment.in','Inward', readonly=True)
        ...
        cls._buttons.update({
                ...
                'wait': {
                    'invisible': ~Eval('state').in_(['draft', 'assigned',
                            'waiting']),
                    'icon': If(Eval('state').in_(['assigned',]),
                        'tryton-back',
    ...
    def wait(cls, productions):
        if len(productions[0].inputs) == 0:
            raise UserError("Input must not be empty")
        else:
            ref_data = ""
            ...
            productions = Production.search([('id', '=',productions[0].id)])
            Production.write(productions, {'reference': ref_data})
    ...
    def assign(cls, productions):
        if len(productions[0].works) == 0:
            raise UserError("Production Must Consist of Works Process")
    ...
    def run(cls, productions):
        ...
        PostProduction.create([{'production':productions[0].id, 'effective_date':Date.today()}])
    ...
    def done(cls, productions):
        ...
        if len(productions[0].outputs) == 0:
            raise UserError("Output Field must not be empty")
        
        if productions[0].postproduction_state == 'rejected' or productions[0].postproduction_state == 'pending':
            raise UserError("Post Production Analysis Must be Approved")
        ...
        for i in productions[0].inputs:
            v1 = i.inward_ref.remaining_qty
            v2 = i.quantity
            qty = v1-v2
            if qty == 0:
                inward = Inward.search([('id', '=', i.inward_ref.id)])
                Inward.write(inward, {'remaining_qty': qty , 'production_state':'done' })
            else:
                inward = Inward.search([('id', '=', i.inward_ref.id)])
                Inward.write(inward, {'remaining_qty': qty })
